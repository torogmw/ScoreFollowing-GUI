...
#include "Follower.h"
Follower::Follower(vector<int>& score_, vector<vector<float> >& probModel_):sampleRate(44100),hopSize(512),frameSize(2048),score(score_),probModel(probModel_)
{
...
void Follower::followingMain(const float* rawAudio)
{
    ...
    resample(rawAudio, 44100, sampleRate, hopSize*44100/sampleRate);
    // observation
    traCount++;
    observation();
    alignment();
...
{
    getFeatures();          // extract features for probablistic model
    pitchTracking();        // track the current pitch
}
